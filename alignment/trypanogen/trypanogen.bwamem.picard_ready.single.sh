...
# Used the below command when calling with Samtools 1.2-98. It however seems that it is not adding the readgroup information for large BAMs. So reverting back to samtools 0.1.19 
#cmd="$bwa_base/bwa mem -R  \"$bam_readgroup_info\" -M -t $bwamem_threads $ref_seq $f1 $f2 | $samtools_base/samtools view -ubhS - | $samtools_base/samtools sort -O bam -@ $bwamem_threads -T $prefix -o $bam -"
