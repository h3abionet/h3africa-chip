...
####################
    ...
    for i in {1..22}; do 
      ...
      if [ ! -f $tbi ]; then
         pstracker_log_path=$logs_dir/trypanogen.hc_dm.$sample_id.$sample_count.$site_name  
         hc_dm_cmd="qsub -N tg.hcd.$sample_count.$site_name -o $logs_dir/trypanogen.hc_dm.$sample_id.$sample_count.$site_name.o -e $logs_dir/trypanogen.hc_dm.$sample_id.$sample_count.$site_name.e -v config=$config,bam=$bam,site=\"$site\",ploidy=$ploidy,vcf=$vcf,pstracker_log_path=$pstracker_log_path -q $queue -l select=1:ncpus=$gatk_hc_threads:mem=${gatk_hc_mem}B -l walltime=$gatk_hc_walltime -M $pbs_status_mailto -m abe trypanogen.hc.dm.single.sh"
 
        if [ $DEBUG -eq 1 ]; then
          echo $hc_dm_cmd
        else 
          hc_dm_job_id=`eval $hc_dm_cmd`
          echo "TrypanoGEN: $sample_id, site: $site_name, hc_dm_job_id: $hc_dm_job_id"
          echo ${hc_dm_cmd} > $logs_dir/trypanogen.hc_dm.$sample_id.$sample_count.$site_name.$hc_dm_job_id.qsub
          cat trypanogen.hc.dm.single.sh > $logs_dir/trypanogen.hc_dm.$sample_id.$sample_count.$site_name.$hc_dm_job_id.sh
          cat $config > $logs_dir/trypanogen.hc_dm.$sample_id.$sample_count.$site_name.$hc_dm_job_id.config
    
          qalter -o $logs_dir/trypanogen.hc_dm.$sample_id.$sample_count.$site_name.$hc_dm_job_id.o $hc_dm_job_id
          qalter -e $logs_dir/trypanogen.hc_dm.$sample_id.$sample_count.$site_name.$hc_dm_job_id.e $hc_dm_job_id
        fi
      else
...
####################
    ...
    if [ $site ]; then
      ...
      # First check if the vcf index has been created has been created before. If it has been created we can skip that site.
      if [ ! -f $tbi ]; then
        pstracker_log_path=$logs_dir/trypanogen.hc_dm.$sample_id.$sample_count.$sample_id.$site_name 
        hc_dm_cmd="qsub -N tg.hcd.$sample_count.X1 -o $logs_dir/trypanogen.hc_dm.$sample_id.$sample_count.$site_name.o -e $logs_dir/trypanogen.hc_dm.$sample_id.$sample_count.$site_name.e -v config=$config,bam=$bam,site=\"$site\",ploidy=$ploidy,vcf=$vcf,pstracker_log_path=$pstracker_log_path -q $queue -l select=1:ncpus=$gatk_hc_threads:mem=${gatk_hc_mem}B -l walltime=$gatk_hc_walltime -M $pbs_status_mailto -m abe trypanogen.hc.dm.single.sh"
...
####################
      ...
      if [ ! -f $tbi ]; then
        pstracker_log_path=$logs_dir/trypanogen.hc_dm.$sample_id.$sample_count.$sample_id.$site_name 
        hc_dm_cmd="qsub -N tg.hcd.$sample_count.X2 -o $logs_dir/trypanogen.hc_dm.$sample_id.$sample_count.$site_name.o -e $logs_dir/trypanogen.hc_dm.$sample_id.$sample_count.$site_name.e -v config=$config,bam=$bam,site=\"$site\",ploidy=$ploidy,vcf=$vcf,pstracker_log_path=$pstracker_log_path -q $queue -l select=1:ncpus=$gatk_hc_threads:mem=${gatk_hc_mem}B -l walltime=$gatk_hc_walltime -M $pbs_status_mailto -m abe trypanogen.hc.dm.single.sh"
...
####################
      ...
      if [ ! -f $tbi ]; then
        pstracker_log_path=$logs_dir/trypanogen.hc_dm.$sample_id.$sample_count.$sample_id.$site_name 
        hc_dm_cmd="qsub -N tg.hcd.$sample_count.Xn -o $logs_dir/trypanogen.hc_dm.$sample_id.$sample_count.$site_name.o -e $logs_dir/trypanogen.hc_dm.$sample_id.$sample_count.$site_name.e -v config=$config,bam=$bam,site=\"$site\",ploidy=$ploidy,vcf=$vcf,pstracker_log_path=$pstracker_log_path -q $queue -l select=1:ncpus=$gatk_hc_threads:mem=${gatk_hc_mem}B -l walltime=$gatk_hc_walltime -M $pbs_status_mailto -m abe trypanogen.hc.dm.single.sh"
...
####################
      ...
      if [ ! -f $tbi ]; then
        pstracker_log_path=$logs_dir/trypanogen.hc_dm.$sample_id.$sample_count.$sample_id.$site_name 
        hc_dm_cmd="qsub -N tg.hcd.$sample_count.Y1 -o $logs_dir/trypanogen.hc_dm.$sample_id.$sample_count.$site_name.o -e $logs_dir/trypanogen.hc_dm.$sample_id.$sample_count.$site_name.e -v config=$config,bam=$bam,site=\"$site\",ploidy=$ploidy,vcf=$vcf,pstracker_log_path=$pstracker_log_path -q $queue -l select=1:ncpus=$gatk_hc_threads:mem=${gatk_hc_mem}B -l walltime=$gatk_hc_walltime -M $pbs_status_mailto -m abe trypanogen.hc.dm.single.sh"
...
####################
    ### Run diploid  on Y_PAR2
      ...
      if [ ! -f $tbi ]; then
        pstracker_log_path=$logs_dir/trypanogen.hc_dm.$sample_id.$sample_count.$sample_id.$site_name 
        hc_dm_cmd="qsub -N tg.hcd.$sample_count.Y2 -o $logs_dir/trypanogen.hc_dm.$sample_id.$sample_count.$site_name.o -e $logs_dir/trypanogen.hc_dm.$sample_id.$sample_count.$site_name.e -v config=$config,bam=$bam,site=\"$site\",ploidy=$ploidy,vcf=$vcf,pstracker_log_path=$pstracker_log_path -q $queue -l select=1:ncpus=$gatk_hc_threads:mem=${gatk_hc_mem}B -l walltime=$gatk_hc_walltime -M $pbs_status_mailto -m abe trypanogen.hc.dm.single.sh"
...
####################
      ...
      if [ ! -f $tbi ]; then
        pstracker_log_path=$logs_dir/trypanogen.hc_dm.$sample_id.$sample_count.$sample_id.$site_name 
        hc_dm_cmd="qsub -N tg.hcd.$sample_count.Yn -o $logs_dir/trypanogen.hc_dm.$sample_id.$sample_count.$site_name.o -e $logs_dir/trypanogen.hc_dm.$sample_id.$sample_count.$site_name.e -v config=$config,bam=$bam,site=\"$site\",ploidy=$ploidy,vcf=$vcf,pstracker_log_path=$pstracker_log_path -q $queue -l select=1:ncpus=$gatk_hc_threads:mem=${gatk_hc_mem}B -l walltime=$gatk_hc_walltime -M $pbs_status_mailto -m abe trypanogen.hc.dm.single.sh"
...
####################
    ...
    if [ $site ]; then
      ...
      if [ ! -f $tbi ]; then
        pstracker_log_path=$logs_dir/trypanogen.hc_dm.$sample_id.$sample_count.$sample_id.$site_name 
        hc_dm_cmd="qsub -N tg.hcd.$sample_count.X -o $logs_dir/trypanogen.hc_dm.$sample_id.$sample_count.$site_name.o -e $logs_dir/trypanogen.hc_dm.$sample_id.$sample_count.$site_name.e -v config=$config,bam=$bam,site=\"$site\",ploidy=$ploidy,vcf=$vcf,pstracker_log_path=$pstracker_log_path -q $queue -l select=1:ncpus=$gatk_hc_threads:mem=${gatk_hc_mem}B -l walltime=$gatk_hc_walltime -M $pbs_status_mailto -m abe trypanogen.hc.dm.single.sh"
...
####################
    ...
    if [ ! -f $tbi ]; then
      pstracker_log_path=$logs_dir/trypanogen.hc_dm.$sample_id.$sample_count.$sample_id.$site_name 
      hc_dm_cmd="qsub -N tg.hcd.$sample_count.M -o $logs_dir/trypanogen.hc_dm.$sample_id.$sample_count.$site_name.o -e $logs_dir/trypanogen.hc_dm.$sample_id.$sample_count.$site_name.e -v config=$config,bam=$bam,site=\"$site\",ploidy=$ploidy,vcf=$vcf,pstracker_log_path=$pstracker_log_path -q $queue -l select=1:ncpus=$gatk_hc_threads:mem=${gatk_hc_mem}B -l walltime=$gatk_hc_walltime -M $pbs_status_mailto -m abe trypanogen.hc.dm.single.sh"
