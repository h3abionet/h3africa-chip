...
for i in {1..22}; do 
  ...
  if ! ([ -f $vqsr_snps_recal ] && [ -f $vqsr_snps_recal.idx ] && [ -f $vqsr_snps_tranches ] && [ -f $vqsr_snps_rscript ]) ; then
    vqsr_cmd="qsub -N trypanogen.vqsr.$site_name -o $logs_dir/trypanogen.vqsr.$site_name.o -e $logs_dir/trypanogen.vqsr.$site_name.e -v config=$config,genotyped_vcf=$genotyped_vcf,site=\"$site\",vqsr_snps_recal=$vqsr_snps_recal,vqsr_snps_tranches=$vqsr_snps_tranches,vqsr_snps_rscript=$vqsr_snps_rscript -q $queue -l nodes=1:ppn=$gatk_vqsr_threads -l walltime=$gatk_vqsr_walltime -M $pbs_status_mailto -m abe trypanogen.vqsr.single.sh"
 
    if [ $DEBUG -eq 1 ]; then
      echo $vqsr_cmd
    else 
      vqsr_job_id=`eval $vqsr_cmd`
      echo "TrypanoGEN: site: $site_name, vqsr_job_id: $vqsr_job_id"
      echo ${vqsr_cmd} > $logs_dir/trypanogen.vqsr.$site_name.$vqsr_job_id.qsub
      cat trypanogen.vqsr.single.sh > $logs_dir/trypanogen.vqsr.$site_name.$vqsr_job_id.sh
      cat $config > $logs_dir/trypanogen.vqsr.$site_name.$vqsr_job_id.config
    
      qalter -o $logs_dir/trypanogen.vqsr.$site_name.$vqsr_job_id.o $vqsr_job_id
      qalter -e $logs_dir/trypanogen.vqsr.$site_name.$vqsr_job_id.e $vqsr_job_id
    fi
  else
