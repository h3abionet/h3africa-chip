...
for i in {1..22}; do 
  ...
  if [ ! -f $cross_impute_ready_tbi ]; then
    prepare_cross_impute_cmd="qsub -N trypanogen.pci.$site_name -o $logs_dir/trypanogen.prepare_for_cross_impute.$site_name.o -e $logs_dir/trypanogen.prepare_for_cross_impute.$site_name.e -v config=$config,apply_vqsr_vcf=$apply_vqsr_vcf,cross_impute_ready_vcf=$cross_impute_ready_vcf,site=\"$site\" -q $queue -l nodes=1:ppn=$prepare_cross_impute_threads -l walltime=$prepare_cross_impute_walltime -M $pbs_status_mailto -m abe trypanogen.prepare_for_cross_impute.single.sh"
 
    if [ $DEBUG -eq 1 ]; then
      echo $prepare_cross_impute_cmd
    else 
      prepare_cross_impute_job_id=`eval $prepare_cross_impute_cmd`
      echo "TrypanoGEN: site: $site_name, prepare_cross_impute_job_id: $prepare_cross_impute_job_id"
      echo ${prepare_cross_impute_cmd} > $logs_dir/trypanogen.prepare_for_cross_impute.$site_name.$prepare_cross_impute_job_id.qsub
      cat trypanogen.prepare_for_cross_impute.single.sh > $logs_dir/trypanogen.prepare_for_cross_impute.$site_name.$prepare_cross_impute_job_id.sh
      cat $config > $logs_dir/trypanogen.prepare_for_cross_impute.$site_name.$prepare_cross_impute_job_id.config
    
      qalter -o $logs_dir/trypanogen.prepare_for_cross_impute.$site_name.$prepare_cross_impute_job_id.o $prepare_cross_impute_job_id
      qalter -e $logs_dir/trypanogen.prepare_for_cross_impute.$site_name.$prepare_cross_impute_job_id.e $prepare_cross_impute_job_id
    fi
  else
