...
for i in {1..22}; do 
  ...
  if [ ! -f $apply_vqsr_tbi ]; then
    apply_vqsr_cmd="qsub -N trypanogen.apply_vqsr.$site_name -o $logs_dir/trypanogen.apply_vqsr.$site_name.o -e $logs_dir/trypanogen.apply_vqsr.$site_name.e -v config=$config,genotyped_vcf=$genotyped_vcf,apply_vqsr_vcf=$apply_vqsr_vcf,site=\"$site\",vqsr_snps_recal=$vqsr_snps_recal,vqsr_snps_tranches=$vqsr_snps_tranches -q $queue -l nodes=1:ppn=$gatk_apply_vqsr_threads -l walltime=$gatk_apply_vqsr_walltime -M $pbs_status_mailto -m abe trypanogen.apply_vqsr.single.sh"
 
    if [ $DEBUG -eq 1 ]; then
      echo $apply_vqsr_cmd
    else 
      apply_vqsr_job_id=`eval $apply_vqsr_cmd`
      echo "TrypanoGEN: site: $site_name, apply_vqsr_job_id: $apply_vqsr_job_id"
      echo ${apply_vqsr_cmd} > $logs_dir/trypanogen.apply_vqsr.$site_name.$apply_vqsr_job_id.qsub
      cat trypanogen.apply_vqsr.single.sh > $logs_dir/trypanogen.apply_vqsr.$site_name.$apply_vqsr_job_id.sh
      cat $config > $logs_dir/trypanogen.apply_vqsr.$site_name.$apply_vqsr_job_id.config
    
      qalter -o $logs_dir/trypanogen.apply_vqsr.$site_name.$apply_vqsr_job_id.o $apply_vqsr_job_id
      qalter -e $logs_dir/trypanogen.apply_vqsr.$site_name.$apply_vqsr_job_id.e $apply_vqsr_job_id
    fi
  else
