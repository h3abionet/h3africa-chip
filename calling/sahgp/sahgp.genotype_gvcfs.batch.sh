...
for i in {1..22}; do 
  ...
  if [ ! -f $tbi ]; then
    genotype_gvcfs_cmd="qsub -N sahgp.gtg.$site_name -o $logs_dir/sahgp.genotype_gvcfs.$site_name.o -e $logs_dir/sahgp.genotype_gvcfs.$site_name.e -v config=$config,gvcf_list=$gvcf_list,site=\"$site\",vcf=$vcf -q $queue -l nodes=1:ppn=$gatk_genotype_gvcfs_threads -l walltime=$gatk_genotype_gvcfs_walltime -M $pbs_status_mailto -m abe sahgp.genotype_gvcfs.single.sh"
 
    if [ $DEBUG -eq 1 ]; then
      echo $genotype_gvcfs_cmd
    else 
      genotype_gvcfs_job_id=`eval $genotype_gvcfs_cmd`
      echo "SAHGP: site: $site_name, genotype_gvcfs_job_id: $genotype_gvcfs_job_id"
      echo ${genotype_gvcfs_cmd} > $logs_dir/sahgp.genotype_gvcfs.$site_name.$genotype_gvcfs_job_id.qsub
      cat sahgp.genotype_gvcfs.single.sh > $logs_dir/sahgp.genotype_gvcfs.$site_name.$genotype_gvcfs_job_id.sh
      cat $config > $logs_dir/sahgp.genotype_gvcfs.$site_name.$genotype_gvcfs_job_id.config
    
      qalter -o $logs_dir/sahgp.genotype_gvcfs.$site_name.$genotype_gvcfs_job_id.o $genotype_gvcfs_job_id
      qalter -e $logs_dir/sahgp.genotype_gvcfs.$site_name.$genotype_gvcfs_job_id.e $genotype_gvcfs_job_id
    fi
  else
