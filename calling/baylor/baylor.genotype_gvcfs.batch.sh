...
for i in {1..22}; do 
  site="-L "$i
  site_name=$i
  ls -1 $genotype_gvcfs_ready_dir/*/*.$site_name.raw.g.vcf.gz > $tmp_dir/$cohort.$site_name.g.vcf.gz.list
...
if `ls -1 $genotype_gvcfs_ready_dir/*/*.$site_name.raw.g.vcf.gz > $tmp_dir/$cohort.$site_name.g.vcf.gz.list 2>/dev/null`; then
  gvcf_list=$tmp_dir/$cohort.$site_name.g.vcf.gz.list
  vcf=$genotype_gvcfs_dir/$cohort.$site_name".vcf.gz"
  tbi=$genotype_gvcfs_dir/$cohort.$site_name".vcf.gz.tbi"
...
ls -1 $genotype_gvcfs_ready_dir/*/*.$site_name.raw.g.vcf.gz > $tmp_dir/$cohort.$site_name.g.vcf.gz.list
gvcf_list=$tmp_dir/$cohort.$site_name.g.vcf.gz.list
vcf=$genotype_gvcfs_dir/$cohort.$site_name".raw.vcf.gz"
tbi=$genotype_gvcfs_dir/$cohort.$site_name".raw.vcf.gz.tbi"
