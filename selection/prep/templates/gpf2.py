...
def parseArguments():
    ...
    parser.add_argument('--good_snps_d', dest='good', action='store',\
                   default = "GOOD",help="Directory with good snps")
    parser.add_argument('--twobeadsnps', dest='twobeadf', action='store',\
                   default="/spaces/scott/chip/alltbds.snps", help='file of SNPs that require two beads')
...
def patchWindows(prob_f,selected,cost,alt_selected,blocks):
    ...
    with open(prob_f) as f: 
        for snp in f:
            [chrom,start,finish]=re.split("[_:-]",snp.rstrip())
            chrom=conv(chrom)
...
tbds = getTwoBeadSNPs(args.twobeadf)
...
print("Reading good SNPs")
good_snps = getGoodSNPs(good_snps_d)
all_keys=[]
